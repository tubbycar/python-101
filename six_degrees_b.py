...
while loopcount<=input_deg:
    for name in current_name:
        degree_friends=my_dict[name]
        for friends in degree_friends:
            short_return_list.append(friends)
        loopcount+=1
        current_name=degree_friends
short_strip_repeats=[]
for item in short_return_list:
    if not(item in short_strip_repeats):
        short_strip_repeats.append(item)
if orig_name in short_strip_repeats:
    short_strip_repeats.remove(orig_name)
final_list=[]
for item in strip_repeats:
    if not(item in short_strip_repeats):
